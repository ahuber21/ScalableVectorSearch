...
CATCH_TEST_CASE(
    "Multi: Selective consolidation under SeqlockSync",
    "[index][vamana][multi][sync][consolidate]"
) {
    // Test that consolidate(labels) only removes specified labels while leaving
    // others searchable. This exercises tracks_pending_label_deletes trait.
    using Distance = svs::distance::DistanceL2;
    using Eltype = float;
    using DataAlloc =
        svs::data::Blocked<svs::lib::Allocator<Eltype>, svs::data::SegmentStable>;
    ...
    // Create labels: 0..num_points-1
    std::vector<size_t> all_labels(num_points);
    std::iota(all_labels.begin(), all_labels.end(), 0);

    const svs::index::vamana::VamanaBuildParameters build_parameters{
        alpha, max_degree, 2 * max_degree, 500, max_degree - 4, true};

    auto index = svs::index::vamana::
        MultiMutableVamanaIndex<Graph, Data, Distance, svs::index::vamana::SeqlockSync>(
            build_parameters, data, all_labels, Distance(), num_threads
        );

    CATCH_REQUIRE(index.size() == num_points);
    CATCH_REQUIRE(index.labelcount() == num_points);

    // Delete the first quarter of labels (will be consolidated selectively).
    const size_t quarter = num_points / 4;
    std::vector<size_t> to_consolidate(quarter);
    std::iota(to_consolidate.begin(), to_consolidate.end(), 0);
    index.delete_entries(to_consolidate);

    CATCH_REQUIRE(index.labelcount() == num_points - quarter);

    // Delete the second quarter (will NOT be consolidated yet).
    std::vector<size_t> to_leave_pending(quarter);
    std::iota(to_leave_pending.begin(), to_leave_pending.end(), quarter);
    index.delete_entries(to_leave_pending);

    CATCH_REQUIRE(index.labelcount() == num_points - 2 * quarter);

    // Selectively consolidate only the first quarter.
    size_t consolidated_count = index.consolidate(to_consolidate);
    CATCH_REQUIRE(consolidated_count == quarter);

    // Verify remaining labels are still searchable. Take search parameters from index
    // and set buffer_config to avoid zero-width window segfault.
    auto search_params = index.search_parameters();
    search_params.buffer_config({10});

    // Query using labels from the second half (untouched by delete or consolidate).
    const size_t half = num_points / 2;
    std::vector<size_t> searchable_labels;
    for (size_t i = half; i < num_points; ++i) {
        searchable_labels.push_back(i);
    }

    auto query_data =
        svs::data::SimpleData<Eltype, N>::load(test_dataset::query_svs_file());
    const size_t num_queries = std::min<size_t>(query_data.size(), 10);
    const size_t num_neighbors = std::min<size_t>(5, searchable_labels.size());

    auto results = svs::QueryResult<size_t>(num_queries, num_neighbors);
    index.search(
        results.view(),
        query_data.cview().slice(svs::threads::UnitRange(0, num_queries)),
        search_params
    );

    // Verify all returned neighbors are from searchable labels.
    for (size_t q = 0; q < num_queries; ++q) {
        for (size_t n = 0; n < num_neighbors; ++n) {
            auto label = results.neighbor(q, n);
            CATCH_REQUIRE(label >= half);
            CATCH_REQUIRE(label < num_points);
        }
    }

    // Consolidate the second quarter - should now succeed.
    size_t second_consolidated = index.consolidate(to_leave_pending);
    CATCH_REQUIRE(second_consolidated == quarter);
}
