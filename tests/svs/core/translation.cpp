...
CATCH_TEST_CASE("Translation Table", "[core][translation]") {
    ...
    CATCH_SECTION("Basic Tests") {
        ...
        CATCH_SECTION("replace_stale_and_insert rejects duplicate live internal ID") {
            // Regression test: validates internal IDs, not just external IDs.
            // Starting state: external 0 -> internal 0 (among others).
            // Attempt to insert external 100 -> internal 0 with nothing stale.
            // Must throw because internal 0 is live, and maps stay unchanged.
            auto add_external = std::vector<uint64_t>{100};
            auto add_internal = std::vector<uint32_t>{0};
            auto never_stale = [](uint32_t) { return false; };
            ...
            // Both maps unchanged: external 0 still maps to internal 0.
            CATCH_REQUIRE(translator.get_internal(0) == 0);
            CATCH_REQUIRE(translator.get_external(0) == 0);
            CATCH_REQUIRE_FALSE(translator.has_external(100));
            check(translator, external_ids, internal_ids);
        }
