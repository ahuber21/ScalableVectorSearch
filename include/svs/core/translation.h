...
class IDTranslator {
  public:
    ...
    ///
    /// Throws if an external or internal ID already maps to a non-stale counterpart.
    ...
    void replace_stale_and_insert(
        ...
        // Validation pass: check external IDs.
        ...
        // Validation pass: check internal IDs.
        for (auto i = int_begin; i != int_end; ++i) {
            auto found = internal_to_external_.find(*i);
            if (found != internal_to_external_.end() && !is_stale(*i)) {
                throw ANNEXCEPTION("Index already contains internal ID {}!", *i);
            }
        }
    ...
    ///
    /// @param i The internal ID to look up.
    /// @param default_val Value returned if the internal ID is not mapped.
    ///
    external_id_type
    ...
    ///
    /// @param e The external ID to look up.
    /// @param default_val Value returned if the external ID is not mapped.
    ///
    internal_id_type
